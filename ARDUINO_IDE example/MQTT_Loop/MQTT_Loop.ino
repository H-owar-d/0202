/*
 ...
 MQTT Continues Publishing example
...
void reconnect() {
    ...
    sprintf(publishTopic, "%s/%s", clientId, PUBLISH_TOPIC);
    sprintf(subscribeTopic, "%s/%s", clientId, SUBSCRIBE_TOPIC);
...
void loop() {
    ...
    if (!(client.connected())) {
      ...
      publishEventCount++;
      sprintf(publishPayload, "!!!Pub Topic %d !!!", publishEventCount);
